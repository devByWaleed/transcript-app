...
function App() {
  // States to manage url, transcript fetching, translation, error & control states
  ...
  const translate = async (text, sourceLang) => {
    ...
    try {
      const response = await fetch(
        `https://ftapi.pythonanywhere.com/translate?sl=${sourceLang}&dl=${target}&text=${encodeURIComponent(text)}`
      );
  ...
  const translateAll = async (transcriptObj, sourceLang) => {
    const entries = Object.entries(transcriptObj);
    const translatedValues = await Promise.all(
      entries.map(async ([key, value]) => {
        const result = await translate(value, sourceLang);
        return [key, result];
      })
    );
    const translatedObj = {};
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.error || 'An error occurred while fetching transcript.');
      } else {
        setTranscript(data.transcript);

        // Translate all transcript entries
        const translated = await translateAll(data.transcript);
  ...
  // CSS styles for blur and loader
  // Using Tailwind inside style tags for blur, fallback with basic CSS for loader.
  return (
    <>
      ...
      {/* Main content: blur if loading */}
      <main
        className={`flex flex-col gap-y-5 h-screen bg-slate-900 transition-all duration-300 ${loading ? 'blur-sm select-none pointer-events-none' : ''
          }`}
      >
        {!showTranscript ? (
          <section
            className="flex flex-col gap-y-5 items-left justify-center p-10 m-auto lg:w-3/4
            bg-slate-400 shadow-[0_0_10px_5px_#e6eef0] rounded hover:shadow-[0_0_15px_10px_#e6eef0]"
          >
            <h1 className="m-auto text-2xl text-slate-900 font-bold underline">
              Yt-Transcriptor Extractor
            </h1>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-y-3">
              <label
                htmlFor="video-link"
                className="font-large m-auto text-white"
              >
                YouTube Video URL
              </label>
              ...
              <button
                className="px-5 py-2 border-none rounded-lg font-bold cursor-pointer 
          ...
          <section className="m-auto p-6 bg-gray-50 rounded-lg shadow-md flex flex-col gap-3 lg:w-3/4">
            <h1 className="m-auto text-2xl text-slate-900 font-bold underline">
              Transcript Explorer: Search & Translate
            </h1>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:gap-4">
              <form
                ...
                onSubmit={async (e) => {
                  ...
                  try {
                    const translated = await translateAll(transcript, sourceLanguage);
                    setTranslation(translated);
                  } catch (err) {
                    setError('Error translating transcript.');
                  }
                }}
              >
                <select
            ...
            <div className="bg-white rounded-lg shadow-inner flex flex-col gap-3 min-h-[250px]">
              ...
              {error && (
                <div className="text-red-700 border-2 border-amber-800 p-2 overflow-y-scroll">
                  {error}
                </div>
              )}
              ...
              {/* <pre className="whitespace-pre-wrap text-sky-600 text-base overflow-y-auto max-h-64">
                {(Object.keys(translation).length > 0 ? Object.entries(translation) : Object.entries(transcript))
                  .filter(([_, text]) =>
        ...
        <footer className="bg-gray-600 text-center p-5 text-white text-sm">
          <p>
            Made by <b className="text-gray-950">Waleed</b> with 🤍 © 2025 All
            rights reserved
          </p>
        </footer>
      </main>
